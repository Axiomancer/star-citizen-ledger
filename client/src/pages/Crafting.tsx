...
export function Crafting() {
  const { data: runs = [] } = useQuery({ queryKey: ['runs', { type: 'crafting' }], queryFn: () => runsApi.list({ type: 'crafting' }) });
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-slate-100">Crafting</h1>
        <p className="text-sm text-slate-500 mt-0.5">All crafting runs — click a run to manage the manufacturing chain</p>
      </div>
      <Card className="p-0">
        <Table>
          <thead><tr><Th>Run</Th><Th>Status</Th><Th>Revenue</Th><Th>Input Cost</Th><Th>Profit</Th><Th>Duration</Th></tr></thead>
          <tbody>
            {(runs as any[]).length === 0 ? (
              <Tr><Td colSpan={6} className="text-center text-slate-500">No crafting runs yet. Create a run with type "crafting".</Td></Tr>
            ) : (
